...
narginchk(1+~isempty(cax),3+~isempty(cax)); 
[numargs,x,y,pvec] = getArgs(args{:});
...
elseif numargs>=2 && (isa(y,'ProbDist') || isa(y,'prob.ProbabilityDistribution'))
   ...
   yy = y; 
   try
       xlab = getString(message('stats:qqplot:xaxisLabel_Distribution', pd.DistName));
   catch
       if isa(pd,'prob.TriangularDistribution') || isa(pd,'prob.PiecewiseLinearDistribution') ||...
               isa(pd,'prob.MultinomialDistribution') ||  isa(pd,'prob.UniformDistribution')
...
cax = newplot(cax);
hrefends = line(cax,mx,my,'LineStyle','-.','Marker','none');
hrefmid = line(cax,qx,qy,'LineStyle','-','Marker','none');
hdat = line(cax,xx,yy,'LineStyle','none','Marker','+');
if length(hdat)==1
...
function [numargs,x,y,pvec] = getArgs(varargin)
% Get number of non-axes original args, and assign to all input variables
numargs = nargin;
if nargin<3
    varargin(3) = {[]};
end
[x,y,pvec] = deal(varargin{1:3});
end
