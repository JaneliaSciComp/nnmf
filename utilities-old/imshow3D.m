function  hIM = imshow3D( Img, disprange )
%modifications by Kaspar Podgorski 2016
...
hIM = imshow(squeeze(Img(:,:,S,:)), [Rmin Rmax]);
...
PlayBtn_Pos = [350 18 40 25];
Speed_Pos = [395 20 20 20];
Export_Pos = [420 18 40 25];
...
set(findobj(gca,'Type', 'Image'),'ButtonDownFcn', @mouseClick);
...
% -=< Figure resize callback function >=-
    function figureResized(object, eventdata)
        ...
        set(hPlayBtn,'Position', PlayBtn_Pos);
        set(hPlaySpeed,'Position', Speed_Pos);
        set(hExportButton,'Position', Export_Pos);
    end

% -=< Slice slider callback function >=-
    function SliceSlider (hObj,event, Img)
        S = round(get(hObj,'Value'));
        set(findobj(gca,'Type', 'Image'),'cdata',squeeze(Img(:,:,S,:)))
        caxis([Rmin Rmax])
        if sno > 1
...
% -=< Coronal view callback function >=-
    ...
    function timerFunc(obj, event)
        speed = max(1,min(100,ceil(str2double(get(hPlaySpeed, 'String')))));
        S = mod(S+speed-1, sno)+1;
        set(findobj(gca,'Type', 'Image'),'cdata',squeeze(Img(:,:,S,:)))
        if sno > 1
            set(shand,'Value',S);
            set(stxthand, 'String', sprintf('Slice# %d / %d',S, sno));
    ...
    function exportButton(obj, event)
        [fn, dr] = uiputfile('*.avi', 'Select a name for the movie');
